...
yfs_client::inum random_inum(bool is_file)
{
  yfs_client::inum i;
  unsigned long r = (rand() << 16) | rand();
  if (is_file) {
    i = 0x80000000 | r;
  } else {
    i = 0x7FFFFFFF & r;
...
// helper function
// looks up fname under pnum directory, returns result in e
yfs_client::status
lookup(yfs_client::inum pnum, std::string fname, struct fuse_entry_param &e)
{
  // some setup
  yfs_client::status ret;
  bzero(&e, sizeof(e));
  ...
  // the file inum
  yfs_client::inum fnum;

  // call yfs_client to lookup
  ret = yfs->lookup(pnum, fname, fnum);
                   ...
                   int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    if (attr->st_size > st.st_size) {
      char c = '\0';
      std::string null_str(attr->st_size - st.st_size, c);
      ret = yfs->write(inum, st.st_size, null_str);
                   ...
                   off_t off, struct fuse_file_info *fi)
{
  ...
  yfs->readdir(inum, dirents);

  for (std::list<yfs_client::dirent>::iterator it = dirents.begin(); 
      it != dirents.end(); ++it) {
