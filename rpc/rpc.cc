        ...
        unsigned int xid_rep, char **b, int *sz)
{
    ...
    // initialize the maxima history if the client hasn't been seen
    if (maxima_xid_rep_.count(clt_nonce) == 0) {
        maxima_xid_rep_[clt_nonce] = xid_rep;
    }
    
    // if preveciously seen max xid_rep already exceeds this xid
    // it's a forgetten request
    if (maxima_xid_rep_[clt_nonce] > xid) {
        return FORGOTTEN;
    }
    
    // update the new maximum
    if (maxima_xid_rep_[clt_nonce] < xid_rep) {
        maxima_xid_rep_[clt_nonce] = xid_rep;
    }

    // iterate through the sliding window to see if we have record
    std::list<reply_t>& window = reply_window_[clt_nonce];
    for (std::list<reply_t>::iterator it = window.begin(); \
            it != window.end(); it++) {
        // seen this one before
        if (it->xid == xid) {
            // cb_present means replied already
            if (it->cb_present) {
                *b = it->buf;
                *sz = it->sz;
                return DONE;
        ...
        // remove anything that's less than or equal to this xid_rep
        if (it->xid <= xid_rep) {
            free(it->buf);
            it = window.erase(it);
        }
    ...
    // it's a new one, create a new reply and put into the sliding window
    reply_t reply(xid);
        ...
        char *b, int sz)
{
    ...
    std::list<reply_t>& window = reply_window_[clt_nonce];
    for (std::list<reply_t>::iterator it = window.begin(); it != window.end(); it++) {
        if (it->xid == xid) {
            it->cb_present = true;
            it->buf = b;
            it->sz = sz;
